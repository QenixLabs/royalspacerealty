...
import { BedDouble, MapPin, RotateCcw, SearchX, SlidersHorizontal, Wallet } from 'lucide-react'
...
export function ProjectExplorer({ projects }: ProjectExplorerProps) {
  ...
  const [location, setLocation] = useState(locationParam || 'all')
  const [bhk, setBhk] = useState('all')
  const [budget, setBudget] = useState('all')
  const [page, setPage] = useState(1)

  useEffect(() => {
    setLocation(locationParam || 'all')
  }, [locationParam])
  ...
  const hasFilters = location !== 'all' || bhk !== 'all' || budget !== 'all'
            ...
            <Select value={location} onValueChange={(v) => v && setLocation(v)}>
              ...
              >
                <SelectValue placeholder="All Locations">
                  {(v: string) => (v === 'all' ? 'All Locations' : v)}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All Locations</SelectItem>
                {location !== 'all' && !locations.includes(location) && (
                  <SelectItem value={location}>Projects in {location}</SelectItem>
                )}
                {locations.map((l) => (
